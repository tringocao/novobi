...
class SchedulerLunch(models.Model):
    ...
    start_hour = fields.Float('Between', oldname='active_from', required=True, default=0)
    ...
    def start_scheduler(self):
        ...
        cron_id_active = self.env['ir.cron'].sudo().search([('name', '=', name_sch)])
        ...
        if (not cron_id_inactive) and (not cron_id_active):
            res = self.createSchedule(name_sch, unit, num_times, start_, interval_number,
